...
import {
    isSelectedGameWithFriendSelector,
    selectedGameAddFriendLoadingSelector,
    selectedGameCategoryIndicesSelector,
    selectedGameCreateLoadingSelector,
    selectedGameExistsRunningGameWithPlayer,
    selectedGameGiveUpLoadingSelector,
    selectedGameIdSelector,
    selectedGameMessagesSelector,
    selectedGameQuestionsSelector,
    selectedGameRemoveFriendLoadingSelector,
    selectedGameRoundStateSelector,
    selectedGameSelector,
    selectedGameShouldUpload,
    sendMessageLoadingSelector,
    uploadRoundLoadingSelector,
...
function Game(): React.ReactElement {
    ...
    const onAddFriend = React.useCallback(
        (userId: string, name: string) => dispatch(addFriend(userId, name)),
        [dispatch],
    );
    const onBack = React.useCallback(() => dispatch(selectGame(null)), [dispatch]);
    const onDeclineGame = React.useCallback(
        (gameId: number) => dispatch(declineGame(gameId)),
        [dispatch],
    );
    const onGiveUp = React.useCallback(
        (gameId: number) => dispatch(giveUpGame(gameId)),
        [dispatch],
    );
    ...
    const onPlay = React.useCallback(
        (gameId: number) => dispatch(startPlaying(gameId, Date.now())),
        [dispatch],
    );
    const onRemoveFriend = React.useCallback(
        (userId: string) => dispatch(removeFriend(userId)),
        [dispatch],
    );
    const requestGame = React.useCallback(
        (gameId: number) => dispatch(loadGame(gameId)),
        [dispatch],
    );
    const sendMessage = React.useCallback(
        (gameId: number, message: string) => dispatch(sendMessageForGame(gameId, message)),
        [dispatch],
    );
    const uploadGameState = React.useCallback(
    ...
    const yourCorrectAnswers = gameRound.reduce(
        (sum, r) => sum + r.yourAnswers.filter(a => a === AnswerType.CORRECT).length,
        0,
    );
    const opponentCorrectAnswers = gameRound.reduce(
        (sum, r) => sum + r.opponentAnswers.filter(a => a === AnswerType.CORRECT).length,
        0,
    );
    return (
        <div>
            ...
            <div className="qd-game_header">
                <div className="qd-game_user">
                    <Avatar avatarCode={user ? user.avatar_code : null} /> Ich
                </div>
                <div className="qd-game_points">
            ...
            <GameRounds
                gameRound={gameRound}
                onQuestionClick={(round, question) => setShowQuestion([round, question])}
            />
            <div className="qd-game_footer">
                {game.state !== GameState.FINISHED &&
                game.state !== GameState.GAVE_UP &&
                game.state !== GameState.ELAPSED ? null : (
                    <Button
                        className="qd-game_again"
                        onClick={() => onNewGame(game.opponent.user_id)}
                ...
                game.state !== GameState.NEW_RANDOM_GAME ? null : (
                    <Button
                ...
                {isFriend ? (
                    <Button
                        onClick={() => onRemoveFriend(game.opponent.user_id)}
                    ...
                    <Button
                        onClick={() => onAddFriend(game.opponent.user_id, game.opponent.name)}
                ...
                <Modal>
                    <ModalDialog>
                        <ModalDialogOptions
                            ...
                            options={['Nein', { text: 'Nochmal spielen', highlighted: true }]}
                            onOptionClick={e => {
                                if (e === 1) {
...
function ShowQuestion({
    ...
    const questionOffset =
        round * QUESTIONS_PER_ROUND * CATEGORIES_PER_ROUND +
        catChoices[round] * CATEGORIES_PER_ROUND;
