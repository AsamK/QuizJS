...
function computePercentage(value: number, total: number): string {
    return total === 0 ? '-' : formatPercentage(value / total);
}
...
export function UserStatistics(): React.ReactElement {
    ...
    return (
        <div className="qd-user-statistics">
            <div className="qd-user-statistics_line">Punkte: {stats.rating}</div>
            <div className="qd-user-statistics_line">Spiele: {stats.n_games_played}</div>
            <div className="qd-user-statistics_line">
                Siege: {stats.n_games_won} (
                {computePercentage(stats.n_games_won, stats.n_games_played)})
            </div>
            <div className="qd-user-statistics_line">
                Unentschieden: {stats.n_games_tied} (
                {computePercentage(stats.n_games_tied, stats.n_games_played)})
            </div>
            <div className="qd-user-statistics_line">
                Niederlagen: {stats.n_games_lost} (
                {computePercentage(stats.n_games_lost, stats.n_games_played)})
            </div>
            <div className="qd-user-statistics_line">
                Perfekte Spiele: {stats.n_perfect_games} (
                {computePercentage(stats.n_perfect_games, stats.n_games_played)})
            </div>
            <div className="qd-user-statistics_line">
            ...
            <div className="qd-user-statistics_categories">
                <h4>Kategorien:</h4>
                {catStats.map(cat => (
                    <div className="qd-user-statistics_category">
