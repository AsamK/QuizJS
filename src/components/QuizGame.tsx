...
import {
    ...
    selectedQuizRoundStateSelector,
...
function QuizGame(): React.ReactElement {
    ...
    const onPlay = React.useCallback(
        (quizId: string) => dispatch(startPlayingQuiz(quizId, Date.now())),
        [dispatch],
    );
    ...
    const yourCorrectAnswers = quizRound.reduce(
        (sum, r) => sum + r.yourAnswers.filter(a => a === AnswerType.CORRECT).length,
        0,
    );
    const opponentCorrectAnswers = quizRound.reduce(
    ...
    return (
        <div>
            ...
            <div className="qd-quiz-game_header">
                <div className="qd-quiz-game_user">
                    <Avatar avatarCode={user ? user.avatar_code : null} /> Ich
                </div>
                <div className="qd-quiz-game_points">
